	...
	public ItemCompass() {
		addPropertyOverride(new ResourceLocation("angle"), new IItemPropertyGetter() {
			...
			public float apply(ItemStack stack, World worldIn, EntityLivingBase entityIn) {
				if (entityIn == null && !stack.isOnItemFrame()) {
					...
					if (worldIn.provider.isSurfaceWorld()) {
						...
						d1 = Maths.positiveModulo(d1 / 360D, 1D);
					...
					return Maths.positiveModulo((float) d0, 1F);
				}
			}
			...
			private double getFrameRotation(EntityItemFrame p_185094_1_) {
				return Maths.wrapDegrees(180 + p_185094_1_.facingDirection.getHorizontalIndex() * 90);
			}
