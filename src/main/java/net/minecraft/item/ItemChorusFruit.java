	...
	public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityLivingBase entityLiving) {
		...
		if (!worldIn.isRemote) {
			...
			for (int i = 0; i < 16; ++i) {
				...
				double d4 = MathHelper.clamp(entityLiving.posY + (double) (entityLiving.getRNG().nextInt(16) - 8), 0D, worldIn.getActualHeight() - 1);
				...
				if (entityLiving.attemptTeleport(d3, d4, d5)) {
					worldIn.playSound(null, d0, d1, d2, SoundEvents.ITEM_CHORUS_FRUIT_TELEPORT, SoundCategory.PLAYERS, 1F, 1F);
					entityLiving.playSound(SoundEvents.ITEM_CHORUS_FRUIT_TELEPORT, 1F, 1F);
					break;
				}
			}
