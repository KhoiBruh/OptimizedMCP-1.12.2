...
public class Explosion {
			...
			for (int k = 0; k < 16; ++k) {
				for (int l = 0; l < 16; ++l) {
					if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
						...
						for (float f1 = 0.3F; f > 0F; f -= 0.22500001F) {
							...
							if (f > 0F && (exploder == null || exploder.canExplosionDestroyBlock(this, world, blockpos, iblockstate, f))) {
								set.add(blockpos);
							}
			...
			if (!entity.isImmuneToExplosions()) {
				...
				if (d12 <= 1D) {
					...
					if (d13 != 0D) {
						...
						double d10 = (1D - d12) * d14;
						entity.attackEntityFrom(DamageSource.causeExplosionDamage(this), (float) ((int) ((d10 * d10 + d10) / 2D * 7D * (double) f3 + 1D)));
						double d11 = d10;
			...
			for (BlockPos blockpos : affectedBlockPositions) {
				...
				if (iblockstate.getMaterial() != Material.AIR) {
					if (block.canDropFromExplosion(this)) {
						block.dropBlockAsItemWithChance(world, blockpos, world.getBlockState(blockpos), 1F / size, 0);
					}
	...
	public EntityLivingBase getExplosivePlacedBy() {
		...
		if (exploder == null) {
			...
			return exploder instanceof EntityLivingBase ? (EntityLivingBase) exploder : null;
		}
	}
