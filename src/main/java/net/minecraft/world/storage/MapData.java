...
public class MapData extends WorldSavedData {
	...
	public void updateVisiblePlayers(EntityPlayer player, ItemStack mapStack) {
		...
		for (int i = 0; i < playersArrayList.size(); ++i) {
			...
			if (!mapdata$mapinfo1.player.isDead && (mapdata$mapinfo1.player.inventory.hasItemStack(mapStack) || mapStack.isOnItemFrame())) {
				if (!mapStack.isOnItemFrame() && mapdata$mapinfo1.player.dimension == dimension && trackingPosition) {
					updateDecorations(MapDecoration.Type.PLAYER, mapdata$mapinfo1.player.world, mapdata$mapinfo1.player.getName(), mapdata$mapinfo1.player.posX, mapdata$mapinfo1.player.posZ, mapdata$mapinfo1.player.rotationYaw);
				}
			} else {
		...
		if (mapStack.isOnItemFrame() && trackingPosition) {
			...
			updateDecorations(MapDecoration.Type.FRAME, player.world, "frame-" + entityitemframe.getEntityId(), blockpos.getX(), blockpos.getZ(), entityitemframe.facingDirection.getHorizontalIndex() * 90);
		}
