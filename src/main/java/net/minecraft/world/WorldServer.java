...
public class WorldServer extends World implements IThreadListener
{
    ...
    public WorldServer(MinecraftServer server, ISaveHandler saveHandlerIn, WorldInfo info, int dimensionId, Profiler profilerIn)
    {
        ...
        playerChunkMap = new PlayerChunkMap(this);
    ...
    public World init()
    {
        ...
        advancementManager = new AdvancementManager(new File(new File(saveHandler.getWorldDirectory(), "data"), "advancements"));
        functionManager = new FunctionManager(new File(new File(saveHandler.getWorldDirectory(), "data"), "functions"), mcServer);
    ...
    protected void updateBlocks()
    {
        ...
        {
            Iterator<Chunk> iterator1 = playerChunkMap.getChunkIterator();
        ...
        else
        {
            ...
            for (Iterator<Chunk> iterator = playerChunkMap.getChunkIterator(); iterator.hasNext(); profiler.endSection())
            {
                ...
                if (flag && flag1 && rand.nextInt(100000) == 0)
                {
                    ...
                    if (isRainingAt(blockpos))
                    {
                        ...
                        if (getGameRules().getBoolean("doMobSpawning") && rand.nextDouble() < (double)difficultyinstance.getAdditionalDifficulty() * 0.01D)
                        {
                            ...
                            addWeatherEffect(new EntityLightningBolt(this, (double)blockpos.getX(), (double)blockpos.getY(), (double)blockpos.getZ(), true));
                        }
                        else
                        {
                            addWeatherEffect(new EntityLightningBolt(this, (double)blockpos.getX(), (double)blockpos.getY(), (double)blockpos.getZ(), false));
                        }
                    }
                }
    ...
    protected BlockPos adjustPosToNearbyEntity(BlockPos pos)
    {
        ...
        if (!list.isEmpty())
        {
            return ((EntityLivingBase)list.get(rand.nextInt(list.size()))).getPosition();
        }
        else
        {
            if (blockpos.getY() == -1)
        ...
        else
        {
            ...
            else
            {
                ...
                for (int j = 0; j < i; ++j)
                {
                    ...
                    if (!runAllPending && nextticklistentry.scheduledTime > worldInfo.getWorldTotalTime())
                    {
                        break;
                    }
    ...
    {
        return canAddEntity(entityIn) ? super.spawnEntity(entityIn) : false;
    }
    ...
    public boolean addWeatherEffect(Entity entityIn)
    {
        ...
        {
            mcServer.getPlayerList().sendToAllNearExcept((EntityPlayer)null, entityIn.posX, entityIn.posY, entityIn.posZ, 512.0D, provider.getDimensionType().getId(), new SPacketSpawnGlobalEntity(entityIn));
            return true;
        }
        else
        {
    ...
    private void sendQueuedBlockEvents()
    {
        while (!blockEventQueue[blockEventCacheIndex].isEmpty())
        {
            ...
            for (BlockEventData blockeventdata : blockEventQueue[i])
            {
                ...
                {
                    mcServer.getPlayerList().sendToAllNearExcept((EntityPlayer)null, (double)blockeventdata.getPosition().getX(), (double)blockeventdata.getPosition().getY(), (double)blockeventdata.getPosition().getZ(), 64.0D, provider.getDimensionType().getId(), new SPacketBlockAction(blockeventdata.getPosition(), blockeventdata.getBlock(), blockeventdata.getEventID(), blockeventdata.getEventParameter()));
                }
            }
