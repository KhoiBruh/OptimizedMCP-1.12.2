	...
	protected void genDecorations(Biome biomeIn, World worldIn, Random random) {
		...
		if (random.nextInt(4) == 0) {
			...
			if (j12 > 0) {
				...
				mushroomBrownGen.generate(worldIn, random, chunkPos.add(i4, k15, k8));
			}
		}
		...
		if (random.nextInt(8) == 0) {
			...
			int k12 = worldIn.getHeight(chunkPos.add(j4, 0, l8)).getY() * 2;

			if (k12 > 0) {
				int l15 = random.nextInt(k12);
				mushroomRedGen.generate(worldIn, random, chunkPos.add(j4, l15, l8));
			}
		}
