...
public class TileEntityEndGateway extends TileEntityEndPortal implements ITickable
{
    ...
    public void update()
    {
        ...
        {
            List<Entity> list = world.<Entity>getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(getPos()));
    ...
    {
        Vec3d vec3d = (new Vec3d((double) getPos().getX(), 0.0D, (double) getPos().getZ())).normalize();
        ...
        if (exitPortal == null)
        {
            ...
            LOGGER.debug("Failed to find suitable block, settling on {}", (Object) exitPortal);
            (new WorldGenEndIsland()).generate(world, new Random(exitPortal.toLong()), exitPortal);
        }
        else
        {
            LOGGER.debug("Found block at {}", (Object) exitPortal);
        }

        exitPortal = findHighestBlock(world, exitPortal, 16, true);
        LOGGER.debug("Creating portal at {}", (Object) exitPortal);
