...
public class BlockCrops extends BlockBush implements IGrowable {
	...
	protected static float getGrowthChance(Block blockIn, World worldIn, BlockPos pos) {
		...
		boolean flag = blockIn == worldIn.getBlockState(blockpos3)
		                                 .getBlock() || blockIn == worldIn.getBlockState(blockpos4).getBlock();
		boolean flag1 = blockIn == worldIn.getBlockState(blockpos1)
		                                  .getBlock() || blockIn == worldIn.getBlockState(blockpos2).getBlock();

		if (flag && flag1) {
			f /= 2F;
		} else {
			boolean flag2 = blockIn == worldIn.getBlockState(blockpos3.north())
			                                  .getBlock() || blockIn == worldIn.getBlockState(blockpos4.north())
			                                                                   .getBlock() || blockIn == worldIn.getBlockState(blockpos4.south())
			                                                                                                    .getBlock() || blockIn == worldIn.getBlockState(blockpos3.south())
			                                                                                                                                     .getBlock();
			...
			if (i >= getMaxAge()) {
				...
				for (int k = 0; k < j; ++k) {
					if (worldIn.rand.nextInt(2 * getMaxAge()) <= i) {
						spawnAsEntity(worldIn, pos, new ItemStack(getSeed()));
					}
				}
			}
		}
