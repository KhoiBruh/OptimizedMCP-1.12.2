				...
				if (options.getSoundLevel(SoundCategory.MASTER) <= 0F) {
					...
					if (sound == SoundHandler.MISSING_SOUND) {
						...
						if (f1 == 0F) {
							...
							try {
								InputStream input = Minecraft.getMinecraft()
								                             .getResourceManager()
								                             .getResource(oggLoc)
								                             .getInputStream();
								if (sound.streaming()) {
									OggAudioStream stream = new OggAudioStream(input);
									src.playStream(stream, f2, f1, p_sound.getX(), p_sound.getY(), p_sound.getZ(), loop, p_sound.getAttenuationType()
									                                                                                            .getType(), f);
								} else {
									int bufferId;
									...
									src.playStatic(bufferId, f2, f1, p_sound.getX(), p_sound.getY(), p_sound.getZ(), loop, p_sound.getAttenuationType()
									                                                                                              .getType(), f);
								}
