...
public class CreativeContainerScreen extends InventoryEffectRenderer {
		...
		if (slotIn == null && selectedTabIndex != CreativeTabs.INVENTORY.getTabIndex() && type != ClickType.QUICK_CRAFT) {
			...
			if (slotIn == destroyItemSlot && flag) {
				...
				if (slotIn == destroyItemSlot) {
					...
					mc.playerController.sendSlotPacket(itemstack1, ((CreativeContainerScreen.CreativeSlot) slotIn).slot.slotNumber);
					...
					mc.player.inventoryContainer.slotClick(slotIn == null ? slotId : ((CreativeContainerScreen.CreativeSlot) slotIn).slot.slotNumber, mouseButton, type, mc.player);
	...
	private void setCurrentCreativeTab(CreativeTabs tab) {
		...
		if (tab == CreativeTabs.INVENTORY) {
			...
			for (int l = 0; l < container.inventorySlots.size(); ++l) {
				Slot slot = new CreativeContainerScreen.CreativeSlot(container.inventorySlots.get(l), l);
	...
	public void handleMouse() throws IOException {
		...
		if (i != 0 && needsScrollBars()) {
			int j = (((CreativeContainerScreen.ContainerCreative) inventorySlots).itemList.size() + 9 - 1) / 9 - 5;
	...
	public static class ContainerCreative extends Container {
		...
		public ContainerCreative(EntityPlayer player) {
			...
			for (int i = 0; i < 5; ++i) {
				for (int j = 0; j < 9; ++j) {
					addSlotToContainer(new CreativeContainerScreen.LockedSlot(CreativeContainerScreen.basicInventory, i * 9 + j, 9 + j * 18, 18 + i * 18));
				}
			}
			...
			for (int k = 0; k < 5; ++k) {
				for (int l = 0; l < 9; ++l) {
					...
					if (i1 >= 0 && i1 < itemList.size()) {
						CreativeContainerScreen.basicInventory.setInventorySlotContents(l + k * 9, itemList.get(i1));
					} else {
						CreativeContainerScreen.basicInventory.setInventorySlotContents(l + k * 9, ItemStack.EMPTY);
					}
				}
			}
		}
