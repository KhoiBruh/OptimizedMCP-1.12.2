...
public class GuiEnchantment extends GuiContainer
{
    ...
    {
        fontRenderer.drawString(nameable.getDisplayName().getUnformattedText(), 12, 5, 4210752);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        for (int k = 0; k < 3; ++k)
        {
            ...
            if (l >= 0 && i1 >= 0 && l < 108 && i1 < 19 && container.enchantItem(mc.player, k))
            {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        ...
        RenderHelper.disableStandardItemLighting();
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        EnchantmentNameParts.getInstance().reseedRandomGenerator((long) container.xpSeed);
