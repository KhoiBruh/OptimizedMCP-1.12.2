...
public class StatsScreen extends Screen implements IProgressMeter {
	...
	private StatsScreen.StatsGeneral generalStats;
	private StatsScreen.StatsItem itemStats;
	private StatsScreen.StatsBlock blockStats;
	private StatsScreen.StatsMobsList mobStats;
	...
	public void initLists() {
		generalStats = new StatsScreen.StatsGeneral(mc);
		generalStats.registerScrollButtons(1, 1);
		itemStats = new StatsScreen.StatsItem(mc);
		itemStats.registerScrollButtons(1, 1);
		blockStats = new StatsScreen.StatsBlock(mc);
		blockStats.registerScrollButtons(1, 1);
		mobStats = new StatsScreen.StatsMobsList(mc);
		mobStats.registerScrollButtons(1, 1);
	}

	public void initButtons() {
		buttons.add(new Button(0, width / 2 + 4, height - 28, 150, 20, I18n.format("gui.done")));
		buttons.add(new Button(1, width / 2 - 160, height - 52, 80, 20, I18n.format("stat.generalButton")));
		Button guibutton = addButton(new Button(2, width / 2 - 80, height - 52, 80, 20, I18n.format("stat.blocksButton")));
		Button guibutton1 = addButton(new Button(3, width / 2, height - 52, 80, 20, I18n.format("stat.itemsButton")));
		Button guibutton2 = addButton(new Button(4, width / 2 + 80, height - 52, 80, 20, I18n.format("stat.mobsButton")));
	...
	abstract class Stats extends GuiSlot {
		...
		protected Stats(Minecraft p_i47550_2_) {
			super(p_i47550_2_, StatsScreen.this.width, StatsScreen.this.height, 32, StatsScreen.this.height - 64, 20);
	...
	class StatsBlock extends StatsScreen.Stats {
		...
		protected void drawSlot(int slotIndex, int xPos, int yPos, int heightIn, int mouseXIn, int mouseYIn, float partialTicks) {
			StatCrafting statcrafting = getSlotStat(slotIndex);
			Item item = statcrafting.getItem();
			drawStatsScreen(xPos + 40, yPos, item);
			renderStat(StatList.getCraftStats(item), xPos + 115, yPos, slotIndex % 2 == 0);
			renderStat(StatList.getObjectUseStats(item), xPos + 165, yPos, slotIndex % 2 == 0);
			renderStat(statcrafting, xPos + 215, yPos, slotIndex % 2 == 0);
			renderStat(StatList.getObjectsPickedUpStats(item), xPos + 265, yPos, slotIndex % 2 == 0);
			renderStat(StatList.getDroppedObjectStats(item), xPos + 315, yPos, slotIndex % 2 == 0);
		}
	...
	class StatsItem extends StatsScreen.Stats {
		...
		protected void drawSlot(int slotIndex, int xPos, int yPos, int heightIn, int mouseXIn, int mouseYIn, float partialTicks) {
			...
			Item item = statcrafting.getItem();
			drawStatsScreen(xPos + 40, yPos, item);
			renderStat(StatList.getObjectBreakStats(item), xPos + 115, yPos, slotIndex % 2 == 0);
			renderStat(StatList.getCraftStats(item), xPos + 165, yPos, slotIndex % 2 == 0);
			renderStat(statcrafting, xPos + 215, yPos, slotIndex % 2 == 0);
			renderStat(StatList.getObjectsPickedUpStats(item), xPos + 265, yPos, slotIndex % 2 == 0);
			renderStat(StatList.getDroppedObjectStats(item), xPos + 315, yPos, slotIndex % 2 == 0);
		}
	...
	class StatsMobsList extends GuiSlot {
		...
		public StatsMobsList(Minecraft mcIn) {
			super(mcIn, StatsScreen.this.width, StatsScreen.this.height, 32, StatsScreen.this.height - 64, fontRenderer.FONT_HEIGHT * 4);
			setShowSelectionBox(false);
		...
		protected int getContentHeight() {
			return getSize() * fontRenderer.FONT_HEIGHT * 4;
		}
		...
		protected void drawSlot(int slotIndex, int xPos, int yPos, int heightIn, int mouseXIn, int mouseYIn, float partialTicks) {
			EntityList.EntityEggInfo entitylist$entityegginfo = mobs.get(slotIndex);
