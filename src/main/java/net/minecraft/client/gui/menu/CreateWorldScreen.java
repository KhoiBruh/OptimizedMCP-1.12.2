...
public class CreateWorldScreen extends Screen {
	...
	public void init() {
		...
		btnGameMode = addButton(new Button(2, width / 2 - 75, 115, 150, 20, I18n.format("selectWorld.gameMode")));
		btnMoreOptions = addButton(new Button(3, width / 2 - 75, 187, 150, 20, I18n.format("selectWorld.moreWorldOptions")));
		btnMapFeatures = addButton(new Button(4, width / 2 - 155, 100, 150, 20, I18n.format("selectWorld.mapFeatures")));
		btnMapFeatures.visible = false;
		btnBonusItems = addButton(new Button(7, width / 2 + 5, 151, 150, 20, I18n.format("selectWorld.bonusItems")));
		btnBonusItems.visible = false;
		btnMapType = addButton(new Button(5, width / 2 + 5, 100, 150, 20, I18n.format("selectWorld.mapType")));
		btnMapType.visible = false;
		btnAllowCommands = addButton(new Button(6, width / 2 - 155, 151, 150, 20, I18n.format("selectWorld.allowCommands")));
		btnAllowCommands.visible = false;
		btnCustomizeType = addButton(new Button(8, width / 2 + 5, 120, 150, 20, I18n.format("selectWorld.customizeType")));
		...
		if (button.enabled) {
			if (button.id == 1) {
				...
				if (WorldType.WORLD_TYPES[selectedIndex] == WorldType.FLAT) {
					mc.displayScreen(new CreateFlatWorldScreen(this, chunkProviderSettingsJson));
				} else {
					mc.displayScreen(new CustomizeWorldScreen(this, chunkProviderSettingsJson));
				}
			}
		}
	}
	...
	private boolean canSelectCurWorldType() {
		...
		if (worldtype != null && worldtype.canBeCreated()) {
			return worldtype != WorldType.DEBUG_ALL_BLOCK_STATES || isShiftDown();
		} else {
			return false;
		}
	}
	...
	protected void keyTyped(char typedChar, int keyCode) {
		...
		(buttons.getFirst()).enabled = !worldNameField.getText().isEmpty();
		calcSaveDirName();
	}
