...
public class FlatPresetsScreen extends Screen {
	...
	static {
		...
		registerPreset(I18n.format("createWorld.customize.preset.overworld"), Item.getItemFromBlock(Blocks.TALLGRASS), BlockTallGrass.Type.GRASS.getMeta(), Biomes.PLAINS, Arrays.asList("village", "biome_1", "decoration", "stronghold", "mineshaft", "dungeon", "lake", "lava_lake"), new FlatLayerInfo(1, Blocks.GRASS), new FlatLayerInfo(3, Blocks.DIRT), new FlatLayerInfo(59, Blocks.STONE), new FlatLayerInfo(1, Blocks.BEDROCK));
	...
	private FlatPresetsScreen.ListSlot list;
	...
	private static void registerPreset(String name, Item icon, int iconMetadata, Biome biome, List<String> features, FlatLayerInfo... layers) {
		...
		FLAT_WORLD_PRESETS.add(new FlatPresetsScreen.LayerItem(icon, iconMetadata, name, flatgeneratorinfo.toString()));
	}
	...
	public void init() {
		...
		list = new FlatPresetsScreen.ListSlot();
		export.setMaxStringLength(1230);
		export.setText(parentScreen.getPreset());
		btnSelect = addButton(new Button(0, width / 2 - 155, height - 28, 150, 20, I18n.format("createWorld.customize.presets.select")));
