...
public class LockIconButton extends Button {
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
		if (visible) {
			...
			if (locked) {
				if (!enabled) {
					guilockiconbutton$icon = LockIconButton.Icon.LOCKED_DISABLED;
				} else if (flag) {
					guilockiconbutton$icon = LockIconButton.Icon.LOCKED_HOVER;
				} else {
					guilockiconbutton$icon = LockIconButton.Icon.LOCKED;
				}
			} else if (!enabled) {
				guilockiconbutton$icon = LockIconButton.Icon.UNLOCKED_DISABLED;
			} else if (flag) {
				guilockiconbutton$icon = LockIconButton.Icon.UNLOCKED_HOVER;
			} else {
				guilockiconbutton$icon = LockIconButton.Icon.UNLOCKED;
			}
