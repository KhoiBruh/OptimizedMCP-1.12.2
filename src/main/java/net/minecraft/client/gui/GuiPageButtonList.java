...
public class GuiPageButtonList extends GuiListExtended
{
    ...
    private void populateComponents()
    {
        for (GuiPageButtonList.GuiListEntry[] aguipagebuttonlist$guilistentry : pages)
        {
            for (int i = 0; i < aguipagebuttonlist$guilistentry.length; i += 2)
            {
                ...
                entries.add(guipagebuttonlist$guientry);

                if (guipagebuttonlist$guilistentry != null && gui != null)
                {
                    componentMap.addKey(guipagebuttonlist$guilistentry.getId(), gui);

                    if (gui instanceof GuiTextField)
                    {
                        editBoxes.add((GuiTextField)gui);
                    }
                }

                if (guipagebuttonlist$guilistentry1 != null && gui1 != null)
                {
                    componentMap.addKey(guipagebuttonlist$guilistentry1.getId(), gui1);

                    if (gui1 instanceof GuiTextField)
                    {
                        editBoxes.add((GuiTextField)gui1);
                    }
                }
            }
        }
    ...
    private void populateEntries()
    {
        ...
        {
            GuiPageButtonList.GuiListEntry guipagebuttonlist$guilistentry = pages[page][i];
            GuiPageButtonList.GuiListEntry guipagebuttonlist$guilistentry1 = i < pages[page].length - 1 ? pages[page][i + 1] : null;
            Gui gui = componentMap.lookup(guipagebuttonlist$guilistentry.getId());
            Gui gui1 = guipagebuttonlist$guilistentry1 != null ? (Gui) componentMap.lookup(guipagebuttonlist$guilistentry1.getId()) : null;
            GuiPageButtonList.GuiEntry guipagebuttonlist$guientry = new GuiPageButtonList.GuiEntry(gui, gui1);
            entries.add(guipagebuttonlist$guientry);
        }
    }
    ...
    private void markVisibility(int p_178060_1_, int p_178060_2_)
    {
        ...
        for (GuiPageButtonList.GuiListEntry guipagebuttonlist$guilistentry1 : pages[p_178060_2_])
        {
            if (guipagebuttonlist$guilistentry1 != null)
            {
                setComponentVisibility(componentMap.lookup(guipagebuttonlist$guilistentry1.getId()), true);
            }
        }
    }
    ...
    public boolean mouseClicked(int mouseX, int mouseY, int mouseEvent)
    {
        ...
        if (i >= 0)
        {
            ...
            if (focusedControl != guipagebuttonlist$guientry.focusedControl && focusedControl != null && focusedControl instanceof GuiTextField)
            {
                ((GuiTextField) focusedControl).setFocused(false);
            }

            focusedControl = guipagebuttonlist$guientry.focusedControl;
        }
    ...
    {
        GuiSlider guislider = new GuiSlider(responder, p_178067_3_.getId(), p_178067_1_, p_178067_2_, p_178067_3_.getCaption(), p_178067_3_.getMinValue(), p_178067_3_.getMaxValue(), p_178067_3_.getInitalValue(), p_178067_3_.getFormatter());
    ...
    {
        GuiListButton guilistbutton = new GuiListButton(responder, p_178065_3_.getId(), p_178065_1_, p_178065_2_, p_178065_3_.getCaption(), p_178065_3_.getInitialValue());
    ...
    private GuiLabel createLabel(int p_178063_1_, int p_178063_2_, GuiPageButtonList.GuiLabelEntry p_178063_3_, boolean p_178063_4_)
    {
        ...
        if (p_178063_4_)
        {
            guilabel = new GuiLabel(mc.fontRenderer, p_178063_3_.getId(), p_178063_1_, p_178063_2_, width - p_178063_1_ * 2, 20, -1);
        }
        else
        {
            guilabel = new GuiLabel(mc.fontRenderer, p_178063_3_.getId(), p_178063_1_, p_178063_2_, 150, 20, -1);
        }
    ...
    {
        if (focusedControl instanceof GuiTextField)
        {
            GuiTextField guitextfield = (GuiTextField) focusedControl;
            ...
            if (!GuiScreen.isKeyComboCtrlV(p_178062_2_))
            {
                if (p_178062_2_ == 15)
                {
                    ...
                    int k = editBoxes.indexOf(focusedControl);
                    ...
                    focusedControl = editBoxes.get(k);
                    guitextfield = (GuiTextField) focusedControl;
                    guitextfield.setFocused(true);
                    int l = guitextfield.y + slotHeight;
            ...
            else
            {
                ...
                int i = editBoxes.indexOf(focusedControl);
                int j = i;

                for (String s1 : astring)
                {
                    GuiTextField guitextfield1 = editBoxes.get(j);
                    guitextfield1.setText(s1);
                    guitextfield1.setResponderEntryValue(guitextfield1.getId(), s1);

                    if (j == editBoxes.size() - 1)
                    {
                        j = 0;
                    }
                    else
    ...
    public static class EditBoxEntry extends GuiPageButtonList.GuiListEntry
    {
        ...
        public EditBoxEntry(int p_i45534_1_, String p_i45534_2_, boolean p_i45534_3_, Predicate<String> p_i45534_4_)
        {
            ...
            filter = (Predicate)MoreObjects.firstNonNull(p_i45534_4_, Predicates.alwaysTrue());
        }
    ...
    public static class GuiEntry implements GuiListExtended.IGuiListEntry
    {
        ...
        private void renderButton(GuiButton p_192635_1_, int p_192635_2_, int p_192635_3_, int p_192635_4_, boolean p_192635_5_, float p_192635_6_)
        {
            ...
            if (!p_192635_5_)
            {
                p_192635_1_.drawButton(client, p_192635_3_, p_192635_4_, p_192635_6_);
            }
        }
        ...
        private void renderLabel(GuiLabel p_178025_1_, int p_178025_2_, int p_178025_3_, int p_178025_4_, boolean p_178025_5_)
        {
            ...
            if (!p_178025_5_)
            {
                p_178025_1_.drawLabel(client, p_178025_3_, p_178025_4_);
            }
        }
    ...
    public static class GuiSlideEntry extends GuiPageButtonList.GuiListEntry
    {
        ...
        public GuiSlideEntry(int p_i45530_1_, String p_i45530_2_, boolean p_i45530_3_, GuiSlider.FormatHelper p_i45530_4_, float p_i45530_5_, float p_i45530_6_, float p_i45530_7_)
        {
            ...
            maxValue = p_i45530_6_;
            initialValue = p_i45530_7_;
        }
