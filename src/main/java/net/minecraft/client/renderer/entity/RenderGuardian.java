...
public class RenderGuardian extends RenderLiving<EntityGuardian> {
		...
		if (super.shouldRender(livingEntity, camera, camX, camY, camZ)) {
			...
			if (livingEntity.hasTargetedEntity()) {
				...
				if (entitylivingbase != null) {
					Vec3d vec3d = getPosition(entitylivingbase, (double) entitylivingbase.height * 0.5D, 1F);
					Vec3d vec3d1 = getPosition(livingEntity, livingEntity.getEyeHeight(), 1F);
	...
	public void doRender(EntityGuardian entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		if (entitylivingbase != null) {
			...
			double d2 = 0.2D;
			double d3 = 0.282D;
			double d4 = 0D + Math.cos(d1 + 2.356194490192345D) * 0.282D;
			double d5 = 0D + Math.sin(d1 + 2.356194490192345D) * 0.282D;
			double d6 = 0D + Math.cos(d1 + (Math.PI / 4D)) * 0.282D;
			double d7 = 0D + Math.sin(d1 + (Math.PI / 4D)) * 0.282D;
			double d8 = 0D + Math.cos(d1 + 3.9269908169872414D) * 0.282D;
			double d9 = 0D + Math.sin(d1 + 3.9269908169872414D) * 0.282D;
			double d10 = 0D + Math.cos(d1 + 5.497787143782138D) * 0.282D;
			double d11 = 0D + Math.sin(d1 + 5.497787143782138D) * 0.282D;
			double d12 = 0D + Math.cos(d1 + Math.PI) * 0.2D;
			double d13 = 0D + Math.sin(d1 + Math.PI) * 0.2D;
			double d14 = 0D + Math.cos(d1 + 0D) * 0.2D;
			double d15 = 0D + Math.sin(d1 + 0D) * 0.2D;
			double d16 = 0D + Math.cos(d1 + (Math.PI / 2D)) * 0.2D;
			double d17 = 0D + Math.sin(d1 + (Math.PI / 2D)) * 0.2D;
			double d18 = 0D + Math.cos(d1 + (Math.PI * 3D / 2D)) * 0.2D;
			double d19 = 0D + Math.sin(d1 + (Math.PI * 3D / 2D)) * 0.2D;
			double d20 = 0D;
			double d21 = 0.4999D;
			double d22 = -1F + f3;
			double d23 = d0 * 2.5D + d22;
			bufferbuilder.pos(d12, d0, d13).tex(0.4999D, d23).color(j, k, l, 255).endVertex();
			bufferbuilder.pos(d12, 0D, d13).tex(0.4999D, d22).color(j, k, l, 255).endVertex();
			bufferbuilder.pos(d14, 0D, d15).tex(0D, d22).color(j, k, l, 255).endVertex();
			bufferbuilder.pos(d14, d0, d15).tex(0D, d23).color(j, k, l, 255).endVertex();
			bufferbuilder.pos(d16, d0, d17).tex(0.4999D, d23).color(j, k, l, 255).endVertex();
			bufferbuilder.pos(d16, 0D, d17).tex(0.4999D, d22).color(j, k, l, 255).endVertex();
			bufferbuilder.pos(d18, 0D, d19).tex(0D, d22).color(j, k, l, 255).endVertex();
			bufferbuilder.pos(d18, d0, d19).tex(0D, d23).color(j, k, l, 255).endVertex();
			double d24 = 0D;
			...
			bufferbuilder.pos(d6, d0, d7).tex(1D, d24 + 0.5D).color(j, k, l, 255).endVertex();
