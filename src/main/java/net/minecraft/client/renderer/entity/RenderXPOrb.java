...
public class RenderXPOrb extends Render<EntityXPOrb> {
	...
	public void doRender(EntityXPOrb entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		if (!renderOutlines) {
			...
			float f = (float) (i % 4 * 16) / 64F;
			float f1 = (float) (i % 4 * 16 + 16) / 64F;
			float f2 = (float) (i / 4 * 16) / 64F;
			...
			bufferbuilder.pos(-0.5D, -0.25D, 0D).tex(f, f3).color(l, 255, j1, 128).normal(0F, 1F, 0F).endVertex();
			bufferbuilder.pos(0.5D, -0.25D, 0D).tex(f1, f3).color(l, 255, j1, 128).normal(0F, 1F, 0F).endVertex();
