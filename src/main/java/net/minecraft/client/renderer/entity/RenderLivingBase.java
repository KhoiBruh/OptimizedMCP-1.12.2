...
public abstract class RenderLivingBase<T extends EntityLivingBase> extends Render<T> {
	...
	protected void renderModel(T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor) {
		...
		if (flag || flag1) {
			...
			if (flag1) {
				GLS.enableBlendProfile(GLS.Profile.TRANSPARENT_MODEL);
			}

			mainModel.render(entitylivingbaseIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor);

			if (flag1) {
				GLS.disableBlendProfile(GLS.Profile.TRANSPARENT_MODEL);
			}
		}
	}
	...
	protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures) {
		...
		if (!flag && !flag1) {
			...
			GLS.activeTexture(OpenGlHelper.defaultTexUnit);
			GLS.enableTexture2D();
			GLS.texEnv(8960, 8704, GL13.GL_COMBINE);
			GLS.texEnv(8960, GL13.GL_COMBINE_RGB, 8448);
			GLS.texEnv(8960, GL13.GL_SOURCE0_RGB, OpenGlHelper.defaultTexUnit);
			GLS.texEnv(8960, GL13.GL_SOURCE1_RGB, GL13.GL_PRIMARY_COLOR);
			GLS.texEnv(8960, GL13.GL_OPERAND0_RGB, 768);
			GLS.texEnv(8960, GL13.GL_OPERAND1_RGB, 768);
			GLS.texEnv(8960, GL13.GL_COMBINE_ALPHA, 7681);
			GLS.texEnv(8960, GL13.GL_SOURCE0_ALPHA, OpenGlHelper.defaultTexUnit);
			GLS.texEnv(8960, GL13.GL_OPERAND0_ALPHA, 770);
			GLS.activeTexture(OpenGlHelper.lightmapTexUnit);
			GLS.enableTexture2D();
			GLS.texEnv(8960, 8704, GL13.GL_COMBINE);
			GLS.texEnv(8960, GL13.GL_COMBINE_RGB, GL13.GL_INTERPOLATE);
			GLS.texEnv(8960, GL13.GL_SOURCE0_RGB, GL13.GL_CONSTANT);
			GLS.texEnv(8960, GL13.GL_SOURCE1_RGB, GL13.GL_PREVIOUS);
			GLS.texEnv(8960, GL13.GL_SOURCE2_RGB, GL13.GL_CONSTANT);
			GLS.texEnv(8960, GL13.GL_OPERAND0_RGB, 768);
			GLS.texEnv(8960, GL13.GL_OPERAND1_RGB, 768);
			GLS.texEnv(8960, GL13.GL_OPERAND2_RGB, 770);
			GLS.texEnv(8960, GL13.GL_COMBINE_ALPHA, 7681);
			GLS.texEnv(8960, GL13.GL_SOURCE0_ALPHA, GL13.GL_PREVIOUS);
			GLS.texEnv(8960, GL13.GL_OPERAND0_ALPHA, 770);
			...
			GLS.activeTexture(GL13.GL_TEXTURE2);
			GLS.enableTexture2D();
			GLS.bindTexture(TEXTURE_BRIGHTNESS.getGlTextureId());
			GLS.texEnv(8960, 8704, GL13.GL_COMBINE);
			GLS.texEnv(8960, GL13.GL_COMBINE_RGB, 8448);
			GLS.texEnv(8960, GL13.GL_SOURCE0_RGB, GL13.GL_PREVIOUS);
			GLS.texEnv(8960, GL13.GL_SOURCE1_RGB, OpenGlHelper.lightmapTexUnit);
			GLS.texEnv(8960, GL13.GL_OPERAND0_RGB, 768);
			GLS.texEnv(8960, GL13.GL_OPERAND1_RGB, 768);
			GLS.texEnv(8960, GL13.GL_COMBINE_ALPHA, 7681);
			GLS.texEnv(8960, GL13.GL_SOURCE0_ALPHA, GL13.GL_PREVIOUS);
	...
	protected void unsetBrightness() {
		...
		GLS.texEnv(8960, GL13.GL_OPERAND0_RGB, 768);
		GLS.texEnv(8960, GL13.GL_OPERAND1_RGB, 768);
		GLS.texEnv(8960, GL13.GL_COMBINE_ALPHA, 8448);
		GLS.texEnv(8960, GL13.GL_SOURCE0_ALPHA, OpenGlHelper.defaultTexUnit);
		GLS.texEnv(8960, GL13.GL_SOURCE1_ALPHA, GL13.GL_PRIMARY_COLOR);
		GLS.texEnv(8960, GL13.GL_OPERAND0_ALPHA, 770);
		GLS.texEnv(8960, GL13.GL_OPERAND1_ALPHA, 770);
		GLS.activeTexture(OpenGlHelper.lightmapTexUnit);
		GLS.texEnv(8960, 8704, GL13.GL_COMBINE);
		GLS.texEnv(8960, GL13.GL_COMBINE_RGB, 8448);
		GLS.texEnv(8960, GL13.GL_OPERAND0_RGB, 768);
		GLS.texEnv(8960, GL13.GL_OPERAND1_RGB, 768);
		GLS.texEnv(8960, GL13.GL_SOURCE0_RGB, 5890);
		GLS.texEnv(8960, GL13.GL_SOURCE1_RGB, GL13.GL_PREVIOUS);
		GLS.texEnv(8960, GL13.GL_COMBINE_ALPHA, 8448);
		GLS.texEnv(8960, GL13.GL_OPERAND0_ALPHA, 770);
		GLS.texEnv(8960, GL13.GL_SOURCE0_ALPHA, 5890);
	...
	protected void applyRotations(T entityLiving, float p_77043_2_, float rotationYaw, float partialTicks) {
		...
		if (entityLiving.deathTime > 0) {
			...
			GLS.rotate(f * getDeathMaxRotation(entityLiving), 0F, 0F, 1F);
		} else {
			...
			if (("Dinnerbone".equals(s) || "Grumm".equals(s)) && (!(entityLiving instanceof EntityPlayer) || ((EntityPlayer) entityLiving).isWearing(PlayerModelParts.CAPE))) {
				GLS.translate(0F, entityLiving.height + 0.1F, 0F);
