...
public class ModelBakery {
	...
	private IBakedModel createRandomModelForVariantList(VariantList variantsIn, String modelLocation) {
		...
		if (variantsIn.variantList().isEmpty()) {
			...
			for (Variant variant : variantsIn.variantList()) {
				ModelBlock modelblock = models.get(variant.modelLocation());

				if (modelblock != null && modelblock.isResolved()) {
					if (modelblock.getElements().isEmpty()) {
						LOGGER.warn("Missing elements for: {}", modelLocation);
					} else {
						IBakedModel ibakedmodel = bakeModel(modelblock, variant.rotation(), variant.uvLock());

						if (ibakedmodel != null) {
							++i;
							weightedbakedmodel$builder.add(ibakedmodel, variant.weight());
						}
					}
				} else {
	...
	private Set<ResourceLocation> getVariantsTextureLocations() {
		...
		for (ModelResourceLocation modelresourcelocation : list) {
			...
			for (Variant variant : variantlist.variantList()) {
				ModelBlock modelblock = models.get(variant.modelLocation());
		...
		for (ModelBlockDefinition modelblockdefinition : multipartVariantMap.keySet()) {
			for (VariantList variantlist1 : modelblockdefinition.getMultipartData().getVariants()) {
				for (Variant variant1 : variantlist1.variantList()) {
	...
	private void loadModels() {
		...
		while (!deque.isEmpty()) {
			...
			try {
				...
				LOGGER.warn("In parent chain: {}; unable to load model: '{}'", JOINER.join(getParentPath(resourcelocation1)), resourcelocation1, exception);
			}
