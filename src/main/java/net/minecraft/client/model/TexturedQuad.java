...
public class TexturedQuad {
	...
	public void draw(BufferBuilder renderer, float scale) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			renderer.pos(positiontexturevertex.vector3D.x() * (double) scale, positiontexturevertex.vector3D.y() * (double) scale, positiontexturevertex.vector3D.z() * (double) scale)
