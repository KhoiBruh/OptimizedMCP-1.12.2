...
public class SearchTree<T> implements ISearchTree<T> {
	...
	public List<T> search(String searchText) {
		...
		if (searchText.indexOf(58) < 0) {
			...
			return list1.isEmpty() ? list : Lists.newArrayList(new MergingIterator<>(list.iterator(), list1.iterator(), numericContents));
		}
	}
	...
	static class MergingIterator<T> extends AbstractIterator<T> {
		...
		public MergingIterator(Iterator<T> leftIn, Iterator<T> rightIn, Object2IntMap<T> numbersIn) {
			...
			rightItr = rightIn;
			numbers = numbersIn;
			left = leftIn.hasNext() ? leftIn.next() : null;
			right = rightIn.hasNext() ? rightIn.next() : null;
		}
		...
		protected T computeNext() {
			if (left == null && right == null) {
				...
				if (i <= 0) {
					left = leftItr.hasNext() ? leftItr.next() : null;
				}

				if (i >= 0) {
					right = rightItr.hasNext() ? rightItr.next() : null;
				}
