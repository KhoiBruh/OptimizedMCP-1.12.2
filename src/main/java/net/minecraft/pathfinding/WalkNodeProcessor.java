		...
		if (d0 - p_186332_5_ > 1.125D) {
			...
			if (pathnodetype == PathNodeType.WALKABLE) {
				...
				if (pathnodetype == PathNodeType.OPEN) {
					AxisAlignedBB axisalignedbb3 = new AxisAlignedBB((double) x - d1 + 0.5D, (double) y + 0.001D, (double) z - d1 + 0.5D, (double) x + d1 + 0.5D, (float) y + entity.height, (double) z + d1 + 0.5D);
