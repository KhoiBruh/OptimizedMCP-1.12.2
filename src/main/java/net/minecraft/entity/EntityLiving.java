...
public abstract class EntityLiving extends EntityLivingBase {
	...
	protected void updateEquipmentIfNeeded(EntityItem itemEntity) {
		...
		if (!itemstack1.isEmpty()) {
			if (entityequipmentslot.getSlotType() == EntityEquipmentSlot.Type.HAND) {
				...
				flag = true;
			} else if (itemstack.getItem() instanceof ItemArmor itemarmor && itemstack1.getItem() instanceof ItemArmor itemarmor1 && !EnchantmentHelper.hasBindingCurse(itemstack1)) {
	...
	protected final void updateEntityActionState() {
		...
		if (isRiding() && getRidingEntity() instanceof EntityLiving entityliving) {
	...
	public final boolean processInitialInteract(EntityPlayer player, EnumHand hand) {
		...
		if (getLeashed() && getLeashHolder() == player) {
			...
			if (itemstack.getItem() == Items.LEAD && canBeLeashedTo(player)) {
				...
				return processInteract(player, hand) || super.processInitialInteract(player, hand);
			}
		}
	}
	...
	public void clearLeashed(boolean sendPacket, boolean dropLead) {
		...
		if (isLeashed) {
			...
			if (!world.isRemote && sendPacket && world instanceof WorldServer) {
				((WorldServer) world).getEntityTracker().sendToTracking(this, new SPacketEntityAttach(this, null));
			}
		}
	}
