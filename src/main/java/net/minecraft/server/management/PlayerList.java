...
public abstract class PlayerList
{
    ...
    public PlayerList(MinecraftServer server)
    {
        ...
        playerStatFiles = Maps.<UUID, StatisticsManagerServer>newHashMap();
        advancements = Maps.<UUID, PlayerAdvancements>newHashMap();
        mcServer = server;
    ...
    public void initializeConnectionToPlayer(NetworkManager netManager, EntityPlayerMP playerIn)
    {
        ...
        playerIn.setWorld(mcServer.getWorld(playerIn.dimension));
        ...
        setPlayerGameTypeBasedOnOther(playerIn, (EntityPlayerMP)null, worldserver);
    ...
    {
        playerDataManager = worldServers[0].getSaveHandler().getPlayerNBTManager();
        worldServers[0].getWorldBorder().addListener(new IBorderListener()
        {
            ...
            {
                sendPacketToAllPlayers(new SPacketWorldBorder(border, SPacketWorldBorder.Action.SET_WARNING_TIME));
            }
            public void onWarningDistanceChanged(WorldBorder border, int newDistance)
            {
                sendPacketToAllPlayers(new SPacketWorldBorder(border, SPacketWorldBorder.Action.SET_WARNING_BLOCKS));
            }
    ...
    {
        NBTTagCompound nbttagcompound = mcServer.worlds[0].getWorldInfo().getPlayerNBTTagCompound();
        ...
        {
            nbttagcompound1 = playerDataManager.readPlayerData(playerIn);
        }
    ...
    {
        playerDataManager.writePlayerData(playerIn);
        StatisticsManagerServer statisticsmanagerserver = playerStatFiles.get(playerIn.getUniqueID());
        ...
        PlayerAdvancements playeradvancements = advancements.get(playerIn.getUniqueID());
    ...
    public void playerLoggedIn(EntityPlayerMP playerIn)
    {
        ...
        sendPacketToAllPlayers(new SPacketPlayerListItem(SPacketPlayerListItem.Action.ADD_PLAYER, new EntityPlayerMP[] {playerIn}));
        WorldServer worldserver = mcServer.getWorld(playerIn.dimension);

        for (int i = 0; i < playerEntityList.size(); ++i)
        {
            playerIn.connection.sendPacket(new SPacketPlayerListItem(SPacketPlayerListItem.Action.ADD_PLAYER, new EntityPlayerMP[] {playerEntityList.get(i)}));
        }
    ...
    public void playerLoggedOut(EntityPlayerMP playerIn)
    {
        ...
        if (entityplayermp == playerIn)
        {
            ...
            advancements.remove(uuid);
        }

        sendPacketToAllPlayers(new SPacketPlayerListItem(SPacketPlayerListItem.Action.REMOVE_PLAYER, new EntityPlayerMP[] {playerIn}));
    }
    ...
    public String allowUserToConnect(SocketAddress address, GameProfile profile)
    {
        ...
        {
            UserListBansEntry userlistbansentry = (UserListBansEntry) bannedPlayers.getEntry(profile);
        ...
        {
            UserListIPBansEntry userlistipbansentry = bannedIPs.getBanEntry(address);
    ...
    public EntityPlayerMP recreatePlayerEntity(EntityPlayerMP playerIn, int dimension, boolean conqueredEnd)
    {
        ...
        playerEntityList.remove(playerIn);
        mcServer.getWorld(playerIn.dimension).removeEntityDangerously(playerIn);
    ...
    public void updatePermissionLevel(EntityPlayerMP player)
    {
        ...
        int i = canSendCommands(gameprofile) ? ops.getPermissionLevel(gameprofile) : 0;
        i = mcServer.isSinglePlayer() && mcServer.worlds[0].getWorldInfo().areCommandsAllowed() ? 4 : i;
        i = commandsAllowedForAll ? 4 : i;
        sendPlayerPermissionLevel(player, i);
    }
    ...
    public void changePlayerDimension(EntityPlayerMP player, int dimensionIn)
    {
        ...
        updateTimeAndWeatherForPlayer(player, worldserver1);
    ...
    public void onTick()
    {
        ...
        {
            sendPacketToAllPlayers(new SPacketPlayerListItem(SPacketPlayerListItem.Action.UPDATE_LATENCY, playerEntityList));
            playerPingIndex = 0;
        }
    }
    ...
    public String[] getOnlinePlayerNames()
    {
        ...
        for (int i = 0; i < playerEntityList.size(); ++i)
        {
            astring[i] = ((EntityPlayerMP) playerEntityList.get(i)).getName();
        }
    ...
    {
        int i = mcServer.getOpPermissionLevel();
        ops.addEntry(new UserListOpsEntry(profile, mcServer.getOpPermissionLevel(), ops.bypassesPlayerLimit(profile)));
        sendPlayerPermissionLevel(getPlayerByUUID(profile.getId()), i);
    }

    public void removeOp(GameProfile profile)
    {
        ops.removeEntry(profile);
        sendPlayerPermissionLevel(getPlayerByUUID(profile.getId()), 0);
    }
    ...
    {
        return mcServer.worlds[0].getSaveHandler().getPlayerNBTManager().getAvailablePlayerDat();
    }
    ...
    private void setPlayerGameTypeBasedOnOther(EntityPlayerMP target, EntityPlayerMP source, World worldIn)
    {
        ...
        {
            target.interactionManager.setGameType(gameType);
        }
    ...
    public void removeAllPlayers()
    {
        for (int i = 0; i < playerEntityList.size(); ++i)
        {
            (playerEntityList.get(i)).connection.disconnect(new TextComponentTranslation("multiplayer.disconnect.server_shutdown", new Object[0]));
        }
    }

    public void sendMessage(ITextComponent component, boolean isSystem)
    {
        mcServer.sendMessage(component);
        ChatType chattype = isSystem ? ChatType.SYSTEM : ChatType.CHAT;
        sendPacketToAllPlayers(new SPacketChat(component, chattype));
    }
    ...
    {
        UUID uuid = playerIn.getUniqueID();
        StatisticsManagerServer statisticsmanagerserver = uuid == null ? null : (StatisticsManagerServer) playerStatFiles.get(uuid);
        ...
        if (statisticsmanagerserver == null)
        {
            ...
            statisticsmanagerserver = new StatisticsManagerServer(mcServer, file2);
            statisticsmanagerserver.readStatFile();
            playerStatFiles.put(uuid, statisticsmanagerserver);
        }
    ...
    public PlayerAdvancements getPlayerAdvancements(EntityPlayerMP p_192054_1_)
    {
        ...
        if (playeradvancements == null)
        {
            File file1 = new File(mcServer.getWorld(0).getSaveHandler().getWorldDirectory(), "advancements");
            File file2 = new File(file1, uuid + ".json");
            playeradvancements = new PlayerAdvancements(mcServer, file2, p_192054_1_);
    ...
    {
        for (PlayerAdvancements playeradvancements : advancements.values())
        {
