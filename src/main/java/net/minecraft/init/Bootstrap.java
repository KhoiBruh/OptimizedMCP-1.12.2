		...
		BlockDispenser.DISPENSE_BEHAVIOR_REGISTRY.putObject(Items.FLINT_AND_STEEL, new Bootstrap.BehaviorDispenseOptional() {
			protected ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
				...
				if (world.isAirBlock(blockpos)) {
					...
					Blocks.TNT.onBlockDestroyedByPlayer(world, blockpos, Blocks.TNT.getDefaultState()
					                                                               .withProperty(BlockTNT.EXPLODE, true));
					world.setBlockToAir(blockpos);
				} else {
					successful = false;
				}
				...
				if (world.isAirBlock(blockpos) && blockskull.canDispenserPlace(world, blockpos, stack)) {
					if (!world.isRemote) {
						...
						if (tileentity instanceof TileEntitySkull) {
							...
							((TileEntitySkull) tileentity).setSkullRotation(enumfacing.getOpposite()
		...
		protected ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
			...
			if (successful) {
				...
				if (itemstack.hasTagCompound()) {
					((TileEntityShulkerBox) tileentity).loadFromNbt(itemstack.getTagCompound()
